...
# Auto-run by .orca/bootstrap via `bun run dev-bootstrap`; this is the
# manual fallback if you bootstrap the package directly.
bun run seed:dev
```
...
| `bun run dev-bootstrap` | Seed dev user + (re)mint JWT + write web/.env             |
