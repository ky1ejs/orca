...
`web/.env` (`VITE_BACKEND_URL`, `VITE_AUTH_TOKEN`) is auto-generated by the worktree's `.orca/bootstrap` script and refreshed every `bun run dev` via the `predev` hook (`backend/src/scripts/dev-bootstrap.ts`). You shouldn't need to edit it by hand. If you paste a non-dev token (e.g., a prod token for debugging), `dev-bootstrap` detects it and leaves it alone.
