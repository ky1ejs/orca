  ...
  return (
    ...
    >
      ...
      <div role="gridcell" className="flex-1 min-w-0 flex items-center gap-1.5">
        <span className="text-gray-500 text-sm font-mono mr-1 flex-shrink-0">{task.displayId}</span>
        <span className="text-gray-100 text-sm truncate">{task.title}</span>
      ...
      {task.assignee && (
        <div role="gridcell" className="flex-shrink-0 mr-2">
          <span className="text-gray-400 text-xs" title={task.assignee.name}>
            {task.assignee.name}
          </span>
        </div>
      )}
